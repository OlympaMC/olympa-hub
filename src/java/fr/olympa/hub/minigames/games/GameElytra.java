...
public class GameElytra extends AGame {
	...
	private Map<Region, Integer> portals = new LinkedHashMap<>();

	private Map<Player, Integer> nextPortal = new HashMap<>();
	private Map<Player, Long> startTime = new HashMap<>();
	private Map<Player, Location> lastKnownLoc = new HashMap<>();
	...
	public GameElytra(OlympaHub plugin, ConfigurationSection configFromFile) throws UnsupportedOperationException {
		...
		List<Region> listReg = new ArrayList<>();
					...
					if (tps > 0)
						p.spigot().sendMessage(ChatMessageType.ACTION_BAR,
								TextComponent.fromLegacyText("§aTemps : §e" +
										new DecimalFormat("#.##").format((System.currentTimeMillis() - tps) / 1000d) + "§as"));
				});
			}
	...
	protected boolean startGame(OlympaPlayerHub p) {
		...
		launchPreGame((Player) p.getPlayer(), startingDelay);
	...
	public void listPortals(CommandContext cmd) {
		...
		List<Region> list = new ArrayList<>(portals.keySet());
